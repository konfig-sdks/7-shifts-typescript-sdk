...
import { readableStreamToString, Client7ShiftsError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://app.7shifts.com/oauth2/token"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new Client7ShiftsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new Client7ShiftsError(e, e.response?.data)
                }
                if (innerError instanceof Client7ShiftsError) {
                    // Got "Client7ShiftsError" from the above try block
                    throw innerError;
                }
